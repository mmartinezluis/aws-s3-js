...
declare class S3Client {
    ...
    uploadFile(file: any, key: string, dirName: string): Promise<object> {
        try {
            ...
            const isoDate: string = new Date(new Date().getTime() + tenMinutes).toISOString();
            const date: string = isoDate.split("T")[0].split("-").join("");  // yyyymmdd
            const formattedIso: string = isoDate.split("-").join("").split(":").join("").split(".").join("");

            const policy: string = this._generatePolicy(isoDate, date, formattedIso);
            const signature: string = this._generateSignature(date, policy);
            const newKey: string = this._generateKey(file, key);
            const fileName: string = dirName ? dirName + "/" + newKey : newKey;
            ...
            return new Promise((resolve, reject) => {
                this._request(this.config.baseUrl, 'POST', formData, function(statusCode: number, xhr: object){
    ...
    private _request(uri: string, method: string, payload: any, callback: function): void {
   ...
   private  _sanityCheckConfig(): void {
    ...
    private  _sanityCheckPayload(payload: any): void {
    ...
    private _generatePolicy(isoDate: string, date: string, formattedIso: string): string {
    ...
    private _generateSignature(date: string, policy: string): string {
    ...
    private _generateKey(file: any, key: string): string {
        let newKey, s;
...
const helpers = {};
helpers.parseKey = function(key: string): string {
